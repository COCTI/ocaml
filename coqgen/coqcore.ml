...
let rec fun_arity e =
  ...
  | Texp_function
      ({fp_kind = Tparam_pat {pat_desc=(Tpat_var _|Tpat_any)}} :: rem, body) ->
        1 + fun_arity {e with exp_desc = Texp_function (rem, body)}
  | Texp_function ([], Tfunction_body e) -> fun_arity e
  | Texp_function ([], Tfunction_cases
...
let rec transl_exp ~vars e =
  ...
  | Texp_function ([], Tfunction_cases {partial; param; cases}) ->
      let pat, exp =
        match cases with
        ...
        List.fold_left
          (fun (args, vars) fp ->
            let pat =
              ...
              | Tparam_pat p -> p
              | _ -> not_allowed ~loc:fp.fp_loc "This function parameter"
            in
            let (parg, vars) = transl_pat ~vars pat in
            let (v, vars) =
              match parg with
              | CTid v -> (v, vars)
              | _ ->
                let name = fresh_name ~vars "v" in
                (name, add_reserved name vars)
            in
            let cty = transl_pat_type ~vars pat in
            ((v, parg, cty) :: args, vars))
          ([], vars) params
      in
      let body =
        match body with
        | Tfunction_body e -> e
        | Tfunction_cases {cases = {c_lhs; c_rhs} :: _} ->
            { e with exp_desc = Texp_function ([], body);
              exp_type = newgenarrow c_lhs.pat_type c_rhs.exp_type }
        | _ -> not_allowed "empty cases"
      in
      let ct = transl_exp ~vars body in
      let ct =
        List.fold_left
          (fun ct (v, parg, _) ->
            match parg with
            | CTid v' when v = v' -> ct
            | CTid "_" -> ct
            | pat -> {ct with pterm = CTmatch (CTid v, None, [pat, ct.pterm])})
          ct args
      in
      let ct =
        match ct.pterm with
          CTabs _ | CTann _ -> ct
        | _ -> if ct.pary >= 2 then ct else transl_exp_type ~vars ct body
      in
      let pterm =
        List.fold_left (fun pt (v, _, cty) -> CTabs (v, Some cty, pt))
          ct.pterm args
      in
