...
Inductive C_point (a : Type) := C_Point (_ : a) (_ : a).

Inductive B_point := B_Point (_ : float) (_ : float).

Inductive A_t6 := A_T6 (_ : A_t3)
with A_t3 := A_T3 (_ : A_t4)
with A_t4 := A_T4 (_ : A_t5)
with A_t5 := A_T5 (_ : A_t6).

Inductive A_t2 := A_T2 (_ : A_t3).

Inductive A_t1 := A_T1 (_ : A_t2).

Inductive A_point := A_Point (_ : PrimInt63.int) (_ : PrimInt63.int).
...
Fixpoint compare_rec (h : nat) (T : ml_type)
  ...
  if h is h.+1 then
    ...
    | ml_A_t1 =>
      fun x y =>
        match x, y with | A_T1 x1, A_T1 y1 => compare_rec ml_A_t2 x1 y1 end
    | ml_A_t2 =>
      fun x y =>
        match x, y with | A_T2 x1, A_T2 y1 => compare_rec ml_A_t3 x1 y1 end
    | ml_A_t3 =>
      fun x y =>
        match x, y with | A_T3 x1, A_T3 y1 => compare_rec ml_A_t4 x1 y1 end
    | ml_A_t4 =>
      fun x y =>
        match x, y with | A_T4 x1, A_T4 y1 => compare_rec ml_A_t5 x1 y1 end
    | ml_A_t5 =>
      fun x y =>
        match x, y with | A_T5 x1, A_T5 y1 => compare_rec ml_A_t6 x1 y1 end
    | ml_A_t6 =>
      fun x y =>
