...
Definition id (T : ml_type) (x : coq_type T) : coq_type T := x.

Definition add (m n : coq_type ml_int) : coq_type ml_int :=
  PrimInt63.add m n.

Definition comp (T : ml_type) (f : coq_type (ml_arrow ml_int T))
  (g : coq_type ml_int) : M (coq_type T) := f (PrimInt63.add g 1%int63).

Definition tuple_example (T T_1 T_2 T_3 : ml_type) (a : coq_type T_3)
  (b : coq_type T_2) (c : coq_type T_1) (d : coq_type T)
  : M (coq_type (ml_pair (ml_pair T_3 (ml_pair T_2 T_1)) T)) :=
  pair (pair a (pair b c)) d.

Eval compute in add 2%int63 3%int63.
