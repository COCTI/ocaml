...
cycle : (T-1 : ml-type) (a b : coq-type T-1) → M (coq-type (ml-rlist T-1))
cycle T-1 a b =
    Do r ← (cnew (ml-rlist T-1) Nil) //
    Do l ← (Do v ← cnew (ml-rlist T-1) (Cons b r) // Ret (Cons a v)) //
    Do _ ← cput (ml-rlist T-1) r l // Ret l

l = Restart it (cycle ml-bool true false)

hd : (T-1 : ml-type) (def : coq-type T-1) (param : coq-type (ml-rlist T-1))
  → coq-type T-1
hd T-1 def param = case param of λ {
                     (Nil) → def
                     ; (Cons a _) → a
                     }

tl : (T-1 : ml-type) (param : coq-type (ml-rlist T-1))
  → M (coq-type (ml-rlist T-1))
tl T-1 param =
    case param of λ {
      (Nil) → Ret Nil
      ; (Cons _ t) → cget (ml-rlist T-1) t
      }
