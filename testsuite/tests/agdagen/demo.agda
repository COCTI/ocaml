...
fac : (h : ℕ) (n : coq-type ml-int) → M (coq-type ml-int)
fac h n =
    case h of λ {
      (suc h) →
        case n of λ {
          ((+ 0)) → Ret (+ 1)
          ; (n-1) → do v ← fac h (_-_ n-1 (+ 1))
                       Ret (_*_ n-1 v)
          }
      ; (_) → FailGas
      }

id-if-not-7 : (x-1 : coq-type ml-int) → M (coq-type ml-int)
id-if-not-7 x-1 =
    case x-1 of λ {
      ((+ 7)) → raise ml-int EqualTo7
      ; (x-2) → Ret x-2
      }

u = Restart it (cnew ml-int (+ 3))

it-1 =
    Restart u
      (do u ← FromW u
          do v ← do v ← cget ml-int u
                    Ret (_+_ v (+ 1))
             cput ml-int u v)
