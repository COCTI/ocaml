...
view ml-t ml-t = just ~ml-t
view ml-t0 ml-t0 = just ~ml-t0
view ml-t1 ml-t1 = just ~ml-t1
view ml-t2 ml-t2 = just ~ml-t2
...
eq-decc _ _ | just ~ml-t | _ = yes refl
eq-decc _ _ | just ~ml-t0 | _ = yes refl
eq-decc _ _ | just ~ml-t1 | _ = yes refl
eq-decc _ _ | just ~ml-t2 | _ = yes refl
...
compare-rec (suc h) {ml-exn} =
  λ x y → case x , y of λ {
    (T-1 x1 , T-1 y1) → compare-rec h x1 y1
    ; (T-1 _ , _) → Ret Lt
    ; (_ , T-1 _) → Ret Gt
...
compare-rec (suc h) {ml-t} =
  λ x y → case x , y of λ {
    (E-1 x1 , E-1 y1) → compare-rec h x1 y1
    }
compare-rec (suc h) {ml-t0} =
  λ x y → case x , y of λ {
    (O , O) → Ret Eq
    ; (O , _) → Ret Lt
    ; (_ , O) → Ret Gt
    ; (T1-1 x1 , T1-1 y1) → compare-rec h x1 y1
    }
compare-rec (suc h) {ml-t1} =
  λ x y → case x , y of λ {
    (T2-1 x1 , T2-1 y1) → compare-rec h x1 y1
    }
compare-rec (suc h) {ml-t2} =
  λ x y → case x , y of λ {
    (T0 x1 , T0 y1) → compare-rec h x1 y1
    }
...
-Eval1 : coq-type ml-t1
-Eval1 = T2-1 x

failwith : (T-2 : ml-type) (s : coq-type ml-string) → M (coq-type T-2)
failwith T-2 s = raise T-2 (Failure s)

length-aux : (h : ℕ) (T-2 : ml-type) (len : coq-type ml-int)
  (param : coq-type (ml-list T-2)) → M (coq-type ml-int)
length-aux h T-2 len param =
    case h of λ {
      (suc h) →
        case param of λ {
          ([]) → Ret len
          ; (_∷_ _ l) → length-aux h T-2 (_+_ len (+ 1)) l
          }
      ; (_) → FailGas
      }

length-1 : (h : ℕ) (T-2 : ml-type) (l : coq-type (ml-list T-2))
  → M (coq-type ml-int)
length-1 h T-2 l = length-aux h T-2 (+ 0) l

cons : (T-2 : ml-type) (a : coq-type T-2) (l : coq-type (ml-list T-2))
  → coq-type (ml-list T-2)
cons T-2 a l = _∷_ a l

hd : (T-2 : ml-type) (param : coq-type (ml-list T-2)) → M (coq-type T-2)
hd T-2 param =
    case param of λ {
      ([]) → failwith T-2 "hd"
      ; (_∷_ a _) → Ret a
...
insert : (h : ℕ) (T-2 : ml-type) (a : coq-type T-2)
  (l : coq-type (ml-list T-2)) → M (coq-type (ml-list T-2))
insert h T-2 a l =
    case h of λ {
      (suc h) →
        case l of λ {
          ([]) → Ret (_∷_ a [])
          ; (_∷_ b l') →
              Do v ← ml-le h T-2 a b //
              if v then Ret (_∷_ a l) else (
                Do v ← insert h T-2 a l' // Ret (_∷_ b v))
          }
      ; (_) → FailGas
      }
...
l =
    Restart it
      (insert h ml-int (+ 3) (_∷_ (+ 1) (_∷_ (+ 2) (_∷_ (+ 4) []))))

isort : (h : ℕ) (T-2 : ml-type) (l-1 : coq-type (ml-list T-2))
  → M (coq-type (ml-list T-2))
isort h T-2 l-1 =
    case h of λ {
      (suc h) →
        case l-1 of λ {
          ([]) → Ret []
          ; (_∷_ a l') → Do v ← isort h T-2 l' // insert h T-2 a v
          }
      ; (_) → FailGas
      }

gcd : (h : ℕ) (m n : coq-type ml-int) → M (coq-type ml-int)
gcd h m n =
    case h of λ {
      (suc h) →
        Do v ← ml-eq h ml-int m (+ 0) //
        if v then Ret n else ( gcd h (_%_ n m) m)
      ; (_) → FailGas
      }

fact-for63 : (n : coq-type ml-int) → M (coq-type ml-int)
fact-for63 n =
    Do v ← (cnew ml-int (+ 1)) //
    Do _ ←
    (Do u ← Ret (+ 1) //
     ...
     forloop u v-1
       (λ i →
          Do v-1 ← (Do v-1 ← cget ml-int v // Ret (_*_ v-1 i)) //
          cput ml-int v v-1)) //
    cget ml-int v
