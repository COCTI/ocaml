...
data _~_ : (T1 T2 : ml-type) → Set where
  ...
  ~ml-rlist : (u1 v1 : ml-type) → ml-rlist u1 ~ ml-rlist v1
...
view (ml-rlist u1) (ml-rlist v1) = just (~ml-rlist u1 v1)
...
ml-rlist-inj : ml-rlist u1 ≡ ml-rlist v1 → (u1 ≡ v1)
ml-rlist-inj refl = refl
...
ml-rlist-cong : (u1 ≡ v1) → ml-rlist u1 ≡ ml-rlist v1
ml-rlist-cong (refl) = refl
...
eq-decc _ _ | just (~ml-rlist u1 v1) | _ = map′ ml-rlist-cong ml-rlist-inj ((eq-decc u1 v1))
...
module MLtypes-aux (M : Set → Set) where
  ...
  data rlist (a : Set) (a-1 : ml-type) : Set where
    Nil : rlist a a-1
    Cons : (a) → (loc (ml-rlist a-1)) → rlist a a-1
  ...
  coq-type T =
      case T of λ {
        ...
        ; (ml-rlist T1) → rlist (coq-type T1) T1
...
cycle : (T-1 : ml-type) (a b : coq-type T-1)
  → M (coq-type (ml-ref (ml-rlist T-1)))
cycle T-1 a b =
    Do r ← (cnew (ml-rlist T-1) Nil) //
    Do _ ←
    (Do v ← (Do v ← cnew (ml-rlist T-1) (Cons b r) // Ret (Cons a v)) //
     cput (ml-rlist T-1) r v) //
    Ret r

rhd : (T-1 : ml-type) (x : coq-type T-1)
  (r : coq-type (ml-ref (ml-rlist T-1))) → M (coq-type T-1)
rhd T-1 x r =
    Do v ← cget (ml-rlist T-1) r //
    case v of λ {
      (Nil) → Ret x
      ; (Cons a _) → Ret a
      }

rtl : (T-1 : ml-type) (r : coq-type (ml-ref (ml-rlist T-1)))
  → M (coq-type (ml-ref (ml-rlist T-1)))
rtl T-1 r =
    Do v ← cget (ml-rlist T-1) r //
    case v of λ {
...
rdrop : (h : ℕ) (T-1 : ml-type) (n : coq-type ml-int)
  (l : coq-type (ml-ref (ml-rlist T-1)))
  → M (coq-type (ml-ref (ml-rlist T-1)))
rdrop h T-1 n l =
    case h of λ {
      (suc h) →
        Do v ← ml-le h ml-int n (+ 0) // if v then Ret l else ( rtl T-1 l)
      ; (_) → FailGas
      }

mkrlist : (h : ℕ) (T-1 : ml-type) (l : coq-type (ml-list T-1))
  (r : coq-type (ml-ref (ml-rlist T-1)))
  → M (coq-type (ml-ref (ml-rlist T-1)))
mkrlist h T-1 l r =
    case h of λ {
      (suc h) →
        case l of λ {
          ([]) → Ret r
          ; (_∷_ a l-1) →
              Do v ← (Do v ← mkrlist h T-1 l-1 r // Ret (Cons a v)) //
              cnew (ml-rlist T-1) v
          }
      ; (_) → FailGas
      }

cyclel : (h : ℕ) (T-1 : ml-type) (a : coq-type T-1)
  (l : coq-type (ml-list T-1)) → M (coq-type (ml-ref (ml-rlist T-1)))
cyclel h T-1 a l =
    Do r ← (cnew (ml-rlist T-1) Nil) //
    Do _ ←
    (Do v ← (Do v ← mkrlist h T-1 l r // Ret (Cons a v)) //
     cput (ml-rlist T-1) r v) //
    Ret r

iappend : (h : ℕ) (T-1 : ml-type) (l1 l2 : coq-type (ml-rlist T-1))
  → M (coq-type (ml-rlist T-1))
iappend h T-1 l1 l2 =
    case h of λ {
      (suc h) →
        case l1 of λ {
          ...
          ; (Cons a l1') →
              Do _ ←
              (Do v ←
               (Do v ← cget (ml-rlist T-1) l1' // iappend h T-1 v l2) //
               cput (ml-rlist T-1) l1' v) //
              Ret l1
          }
      ; (_) → FailGas
      }
