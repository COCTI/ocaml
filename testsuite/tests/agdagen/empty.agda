...
max-v : (h : ℕ) (T-1 : ml-type) (x y : coq-type T-1) → M (coq-type T-1)
max-v h T-1 x y = do v ← ml-lt h T-1 x y
                     if v then Ret x else ( Ret y)

abs-v : (h : ℕ) (x : coq-type ml-int) → M (coq-type ml-int)
abs-v h x = do v ← ml-lt h ml-int x (+ 0)
               if v then Ret (-_ x) else ( Ret x)

norm : (h : ℕ) (p : coq-type ml-point) → M (coq-type ml-int)
norm h p =
    case p of λ {
      (P x y) → do v ← abs-v h y
                     do v-1 ← abs-v h x
                        max-v h ml-int v-1 v
      }

distance : (h : ℕ) (p1 p2 : coq-type ml-point) → M (coq-type ml-int)
distance h p1 p2 =
    case p1 of λ {
      (P x1 y1) →
        case p2 of λ {
          (P x2 y2) → norm h (P (_-_ x1 x2) (_-_ y1 y2))
          }
      }

square : (x : coq-type ml-int) → coq-type ml-int
square x = _*_ x x
