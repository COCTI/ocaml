...
let rec unify (env:Env.t ref) t1 t2 =
  ...
  try
    ...
    | (Tconstr _, Tconstr _, _, _) when Env.has_local_constraints !env ->
        unify2_rec env t1 t2
    | _ ->
        unify2 env t1 t2
    end;
...
and unify2 env t1 t2 = unify2_expand env t1 t2

and unify2_rec env t1 t2 =
  if unify_eq t1 t2 then () else
  ...
  | (Tconstr (p1, tl1, a1), Tconstr (p2, tl2, a2)) ->
      ...
      then begin
        ...
        then unify2_rec env t1 (try_expand_safe !env t2)
        else unify2_rec env (try_expand_safe !env t1) t2
  | _ ->
      raise Cannot_expand
  with Cannot_expand ->
    unify2_expand env t1 t2

and unify2_expand env t1 t2 =
