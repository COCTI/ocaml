...
let unify_head_only ~refine loc renv ty constr =
  let path = cstr_type_path constr in
  let env = !renv in
  let decl = Env.find_type path env in
  let ty' = Ctype.newconstr env path (Ctype.instance_list env decl.type_params) in
  unify_pat_types_refine ~refine loc renv ty' ty
  ...
  List.fold_left (fun as_ty (extra, _loc, _attrs) ->
    ...
    | Tpat_constraint cty ->
      (* [generic_instance] can only be used if the variables of the original
         ...
         [instance ty] and [ty].  *)
      let ty =
        with_local_level env ~post:generalize_structure
          (fun env -> instance env cty.ctyp_type)
      in
      (* This call to unify can't fail since the pattern is well typed. *)
      unify_pat_types p.pat_loc env (instance env as_ty) (instance env ty);
      ty
  ) as_ty p.pat_extra
...
and build_as_type_aux (env : Env.t) p =
  ...
  | Tpat_construct(_, cstr, pl, vto) ->
      ...
      let ty_args, ty_res, _ =
        instance_constructor env Keep_existentials_flexible cstr
      in
  ...
  | Tpat_variant(l, p', _) ->
      ...
      newty env (Tvariant (create_row ~fields ~more:(newvar env)
                             ~name:None ~fixed:None ~closed:false))
  | Tpat_record (lpl,_) ->
      let lbl = snd3 (List.hd lpl) in
      if lbl.lbl_private = Private then p.pat_type else
      let ty = newvar env in
      let ppl = List.map (fun (_, l, p) -> l.lbl_pos, p) lpl in
      let do_label lbl =
        let _, ty_arg, ty_res = instance_label env false lbl in
  ...
  | Tpat_or(p1, p2, row) ->
      ...
      | Some row ->
          ...
          newty env (Tvariant (create_row ~fields ~fixed ~name
                                 ~closed:false ~more:(newvar env)))
      end
...
let solve_Ppat_poly_constraint env loc sty expected_ty =
  ...
  unify_pat_types loc env ty (instance env expected_ty);
  pattern_force := force :: !pattern_force;
  match get_desc ty with
  | Tpoly (body, tyl) ->
      let _, ty' =
        with_level env ~level:generic_level
          (fun env -> instance_poly env ~keep_names:true false tyl body)
      in
      (cty, ty, ty')
  | _ -> assert false

let solve_Ppat_alias env pat =
  with_local_level env ~post:generalize (fun env -> build_as_type env pat)
...
let solve_constructor_annotation renv name_list sty ty_args ty_ex =
  ...
  let ids =
    List.map
      (fun name ->
        ...
        let (id, new_env) =
          Env.enter_type ~scope:expansion_scope name.txt decl !renv in
        renv := new_env;
        {name with txt = id})
      name_list
  in
  let env = !renv in
  let cty, ty, force =
    with_local_level env ~post:(fun env (_,ty,_) -> generalize_structure env ty)
      (fun env -> Typetexp.transl_simple_type_delayed env sty)
  in
  pattern_force := force :: !pattern_force;
  let ty_args =
    let ty1 = instance env ty and ty2 = instance env ty in
    match ty_args with
      [] -> assert false
    | [ty_arg] ->
        ...
        [ty2]
    | _ ->
        unify_pat_types cty.ctyp_loc env ty1 (newty env (Ttuple ty_args));
...
let solve_Ppat_construct ~refine renv loc constr no_existentials
  ...
  let unify_res ty_res expected_ty =
    let refine =
      ...
      | _ -> refine
    in
    unify_pat_types_return_equated_pairs ~refine loc renv ty_res expected_ty
  in

  let ty_args, equated_types, existential_ctyp =
    with_local_level_iter ~post: generalize_structure !renv begin fun env ->
      (* renv' is renv with a raised level; we add type names to it *)
      let old_renv = renv and renv = ref env in
      let expected_ty = instance env expected_ty in
      let expansion_scope = get_gadt_equations_level () in
      let ty_args, ty_res, equated_types, existential_ctyp =
        match existential_styp with
          None ->
            let ty_args, ty_res, _ =
              instance_constructor env
      ...
      if constr.cstr_existentials <> [] then
        lower_variables_only !renv expansion_scope ty_res;

      (* the added names are put back to old_renv,
         keeping the original levels *)
      old_renv := Env.copy_levels ~from:!old_renv !renv;
  ...
  if !Clflags.principal && refine = None then begin
    ...
    try
      TypePairs.iter
        (fun (t1, t2) ->
          generalize_structure !renv t1;
          generalize_structure !renv t2;
...
let solve_Ppat_record_field ~refine loc renv label label_lid record_ty =
  with_local_level_iter !renv ~post:generalize_structure begin fun env ->
    let old_renv = renv and renv = ref env in
    let (_, ty_arg, ty_res) = instance_label env false label in
    begin try
      unify_pat_types_refine ~refine loc renv ty_res (instance env record_ty)
    with Error(_loc, _env, Pattern_type_clash(err, _)) ->
      raise(Error(label_lid.loc, !renv,
...
let solve_Ppat_constraint loc env sty expected_ty =
  let cty, ty, force =
    with_local_level env
      ~post:(fun env (_, ty, _) -> generalize_structure env ty)
      (fun env -> Typetexp.transl_simple_type_delayed env sty)
  in
  pattern_force := force :: !pattern_force;
  let ty, expected_ty' = instance env ty, ty in
  unify_pat_types loc env ty (instance env expected_ty);
  (cty, ty, expected_ty')

let solve_Ppat_variant ~refine loc renv tag no_arg expected_ty =
  ...
  if tag <> Parmatch.some_private_tag then
    unify_pat_types_refine ~refine loc renv
      (newgenty(Tvariant row)) expected_ty;
  (arg_type, make_row (newvar !renv), instance !renv expected_ty)

(* Building the or-pattern corresponding to a polymorphic variant type *)
let build_or_pat env loc lid =
  let path, decl = Env.lookup_type ~loc:lid.loc lid.txt env in
  let tyl = List.map (fun _ -> newvar env) decl.type_params in
  let row0 =
    let ty = expand_head env (newty env (Tconstr(path, tyl, ref Mnil))) in
    match get_desc ty with
  ...
  let ty = newty env (Tvariant (make_row (newvar env))) in
  let gloc = {loc with Location.loc_ghost=true} in
  let row' = ref (make_row (newvar env)) in
  let pats =
    List.map
      (fun (l,p) ->
...
let rec type_pat
  : type k . k pattern_category ->
      ...
      renv: Env.t ref -> Parsetree.pattern -> type_expr -> k general_pattern
  = fun category ~no_existentials ~renv sp expected_ty ->
  Builtin_attributes.warning_scope sp.ppat_attributes
    (fun () ->
       type_pat_aux category ~no_existentials ~renv sp expected_ty
    )

and type_pat_aux
  : type k . k pattern_category -> no_existentials:_ ->
         renv:_ -> _ -> _ -> k general_pattern
  = fun category ~no_existentials ~renv sp expected_ty ->
  let type_pat category ?(renv=renv) =
    type_pat category ~no_existentials ~renv
  in
  let loc = sp.ppat_loc in
  let solve_expected (x : pattern) : pattern =
    unify_pat ~sdesc_for_hint:sp.ppat_desc !renv x (instance !renv expected_ty);
    x
  in
  ...
  match sp.ppat_desc with
    Ppat_any ->
      rvp {
        ...
        pat_type = instance !renv expected_ty;
        pat_attributes = sp.ppat_attributes;
        pat_env = !renv }
  | Ppat_var name ->
      let ty = instance !renv expected_ty in
      ...
      rvp {
        ...
        pat_env = !renv }
  | Ppat_unpack name ->
      let t = instance !renv expected_ty in
  ...
  | Ppat_constraint(
      ...
      let cty, ty, ty' =
        solve_Ppat_poly_constraint !renv lloc sty expected_ty in
  ...
  | Ppat_alias(sq, name) ->
      ...
      let ty_var = solve_Ppat_alias !renv q in
      let id =
      ...
      rvp { pat_desc = Tpat_alias(q, id, name);
            ...
            pat_env = !renv }
  | Ppat_constant cst ->
      let cst = constant_or_raise !renv loc cst in
      rvp @@ solve_expected {
        pat_desc = Tpat_constant cst;
        pat_loc = loc; pat_extra = [];
        pat_type = type_constant !renv cst;
        pat_attributes = sp.ppat_attributes;
        pat_env = !renv }
  ...
  | Ppat_interval _ ->
      raise (Error (loc, !renv, Invalid_interval))
  | Ppat_tuple spl ->
      assert (List.length spl >= 2);
      let expected_tys =
        solve_Ppat_tuple ~refine:None loc renv spl expected_ty in
      let pl = List.map2 (type_pat Value) spl expected_tys in
      rvp {
        pat_desc = Tpat_tuple pl;
        pat_loc = loc; pat_extra=[];
        pat_type = newty !renv (Ttuple(List.map (fun p -> p.pat_type) pl));
        pat_attributes = sp.ppat_attributes;
        pat_env = !renv }
  | Ppat_construct(lid, sarg) ->
      let expected_type =
        match extract_concrete_variant !renv expected_ty with
        | Variant_type(p0, p, _) ->
        ...
        | Not_a_variant_type ->
            ...
            raise (Error (loc, !renv, error))
      in
      let constr =
        let candidates =
          Env.lookup_all_constructors Env.Pattern ~loc:lid.loc lid.txt !renv in
        wrap_disambiguate "This variant pattern is expected to have"
          (mk_expected expected_ty)
          (Constructor.disambiguate Env.Pattern lid !renv expected_type)
          candidates
      in
      ...
      | Some r, (_ :: _ as exs)  ->
          ...
          raise (Error (loc, !renv, Unexpected_existential (r, name, exs)))
      end;
      let sarg', existential_styp =
        ...
        | Some ([], sp) ->
            Some sp, None
        | Some (_, sp) ->
            raise (Error (sp.ppat_loc, !renv, Missing_type_constraint))
      in
      let sargs =
        match sarg' with
          None -> []
      ...
      if List.length sargs <> constr.cstr_arity then
        raise(Error(loc, !renv, Constructor_arity_mismatch(lid.txt,
                                     constr.cstr_arity, List.length sargs)));

      let (ty_args, existential_ctyp) =
        solve_Ppat_construct ~refine:None renv loc constr no_existentials
          existential_styp expected_ty
      in
      ...
      let rec check_non_escaping p =
        ...
        | Ppat_constraint _ ->
            raise (Error (p.ppat_loc, !renv, Inlined_record_escape))
        | _ ->
            ()
      in
  ...
  | Ppat_variant(tag, sarg) ->
      ...
      let arg_type, row, pat_type =
        solve_Ppat_variant ~refine:None loc renv tag constant expected_ty in
      let arg =
  ...
  | Ppat_record(lid_sp_list, closed) ->
      ...
      let expected_type, record_ty =
        match extract_concrete_record !renv expected_ty with
        | Record_type(p0, p, _) ->
            let ty = generic_instance expected_ty in
            Some (p0, p, is_principal expected_ty), ty
        | Maybe_a_record_type -> None, newvar !renv
        | Not_a_record_type ->
      ...
      let type_label_pat (label_lid, label, sarg) =
        let ty_arg =
          solve_Ppat_record_field ~refine:None loc renv label label_lid
            record_ty in
      ...
      let make_record_pat lbl_pat_list =
        check_recordpat_labels loc lbl_pat_list closed;
        {
          ...
          pat_loc = loc; pat_extra = [];
          pat_type = instance !renv record_ty;
          pat_attributes = sp.ppat_attributes;
          pat_env = !renv;
        }
      in
      let lbl_a_list =
        wrap_disambiguate "This record pattern is expected to have"
          (mk_expected expected_ty)
          (type_label_a_list loc false !renv Env.Projection
             type_label_pat expected_type)
          lid_sp_list
      in
      rvp @@ solve_expected (make_record_pat lbl_a_list)
  | Ppat_array spl ->
      let ty_elt = solve_Ppat_array ~refine:None loc renv expected_ty in
  ...
  | Ppat_or(sp1, sp2) ->
      ...
      let outer_lev = Env.current_level !renv in
      (* Introduce a new scope using with_local_level without generalizations *)
      let env1, p1, p1_variables, p1_module_variables, env2, p2 =
        with_local_level !renv begin fun env ->
          let lev = Env.current_level env in
          gadt_equations_level := Some lev;
          let type_pat_rec renv sp = type_pat category sp expected_ty ~renv in
          let renv1 = ref env in
          let p1 = type_pat_rec renv1 sp1 in
          let p1_variables = !pattern_variables in
          let p1_module_variables = !module_variables in
          pattern_variables := initial_pattern_variables;
          module_variables := initial_module_variables;
          let renv2 = ref env in
          let p2 = type_pat_rec renv2 sp2 in
          (!renv1, p1, p1_variables, p1_module_variables, !renv2, p2)
        end
      in
      ...
      let alpha_env =
        enter_orpat_variables loc !renv p1_variables p2_variables in
      ...
      rp { pat_desc = Tpat_or (p1, p2, None);
           ...
           pat_env = !renv }
  | Ppat_lazy sp1 ->
      let nv = solve_Ppat_lazy ~refine:None loc renv expected_ty in
  ...
  | Ppat_constraint(sp, sty) ->
      ...
      let cty, ty, expected_ty' =
        solve_Ppat_constraint loc !renv sty expected_ty in
  ...
  | Ppat_type lid ->
      let (path, p) = build_or_pat !renv loc lid in
      pure category @@ solve_expected
        { p with pat_extra = (Tpat_type (path, lid), loc, sp.ppat_attributes)
        :: p.pat_extra }
  | Ppat_open (lid,p) ->
      let path, new_env =
        !type_open Asttypes.Fresh !renv sp.ppat_loc lid in
      renv := new_env;
      let p = type_pat category ~renv p expected_ty in
      let new_env = !renv in
      begin match Env.remove_last_open path new_env with
      | None -> assert false
      | Some closed_env -> renv := closed_env
      end;
...
let with_equations_level env ?(eqn_lev=Env.current_level env) f =
  Misc.protect_refs [Misc.R (gadt_equations_level, Some eqn_lev)] f

let type_pat category ?no_existentials ?eqn_lev renv sp expected_ty =
  with_equations_level !renv ?eqn_lev
    (fun () -> type_pat category ~no_existentials ~renv sp expected_ty)
...
let type_pattern category ~eqn_lev env spat expected_ty =
  reset_pattern true;
  let new_env = ref env in
  let pat = type_pat category ~eqn_lev new_env spat expected_ty in
...
let type_class_arg_pattern cl_num val_env met_env l spat =
  ...
  if is_optional l then unify_pat val_env pat
      (type_option val_env (newvar val_env));
...
let rec check_counter_example_pat ~info ~renv tp expected_ty k =
  let check_rec ?(info=info) ?(renv=renv) =
    check_counter_example_pat ~info ~renv in
  let loc = tp.pat_loc in
  let refine = Some true in
  let solve_expected (x : pattern) : pattern =
    unify_pat_types_refine ~refine x.pat_loc renv x.pat_type
      (instance !renv expected_ty);
    x
  in
  (* "make pattern" and "make pattern then continue" *)
  let mp ?(pat_type = expected_ty) desc =
    { pat_desc = desc; pat_loc = loc; pat_extra=[];
      pat_type = instance !renv pat_type;
      pat_attributes = []; pat_env = !renv } in
  ...
  match tp.pat_desc with
    Tpat_any | Tpat_var _ ->
      ...
      begin match Parmatch.pats_of_type !renv expected_ty with
      | [] -> raise Empty_branch
      | [{pat_desc = Tpat_any}] -> k' ()
  ...
  | Tpat_constant cst ->
      let cst = constant_or_raise !renv loc (Untypeast.constant cst) in
      k @@ solve_expected
             (mp (Tpat_constant cst) ~pat_type:(type_constant !renv cst))
  | Tpat_tuple tpl ->
      assert (List.length tpl >= 2);
      let expected_tys = solve_Ppat_tuple ~refine loc renv tpl expected_ty in
  ...
  | Tpat_construct(cstr_lid, constr, targs, _) ->
      ...
      let (ty_args, existential_ctyp) =
        solve_Ppat_construct ~refine renv loc constr None None expected_ty
      in
      map_fold_cont
  ...
  | Tpat_variant(tag, targ, _) ->
      ...
      let arg_type, row, pat_type =
        solve_Ppat_variant ~refine loc renv tag constant expected_ty in
      let k arg =
  ...
  | Tpat_record(fields, closed) ->
      ...
      let type_label_pat (label_lid, label, targ) k =
        let ty_arg =
          solve_Ppat_record_field ~refine loc renv label label_lid record_ty in
  ...
  | Tpat_array tpl ->
      let ty_elt = solve_Ppat_array ~refine loc renv expected_ty in
  ...
  | Tpat_or(tp1, tp2, _) ->
      ...
      let state = save_state renv in
      let split_or tp =
        let type_alternative pat =
          set_state state renv; check_rec pat expected_ty k in
        find_valid_alternative type_alternative tp
      in
      if must_split then split_or tp else
      let check_rec_result renv tp : (_, abort_reason) result =
        let info = enter_nonsplit_or info in
        match check_rec ~info tp expected_ty ~renv (fun x -> x) with
        | res -> Ok res
        | exception Need_backtrack -> Error Adds_constraints
        | exception Empty_branch -> Error Empty
      in
      let p1 = check_rec_result (ref !renv) tp1 in
      let p2 = check_rec_result (ref !renv) tp2 in
  ...
  | Tpat_lazy tp1 ->
      let nv = solve_Ppat_lazy ~refine loc renv expected_ty in
      (* do not explode under lazy: PR#7421 *)
      check_rec ~info:(no_explosion info) tp1 nv
        (fun p1 -> mkp k (Tpat_lazy p1))

let check_counter_example_pat
    ~counter_example_args ?eqn_lev renv tp expected_ty =
  with_equations_level !renv ?eqn_lev begin fun () ->
    check_counter_example_pat
      ~info:counter_example_args ~renv tp expected_ty (fun x -> x)
  end

(* this function is passed to Partial.parmatch
   to type check gadt nonexhaustiveness *)
let partial_pred ?eqn_lev ~splitting_mode ?(explode=0) env expected_ty p =
  let renv = ref env in
  let state = save_state renv in
  ...
  try
    ...
    let typed_p =
      check_counter_example_pat
        ?eqn_lev ~counter_example_args renv p expected_ty in
    set_state state renv;
...
let check_partial ?eqn_lev env expected_ty loc cases =
  let explode = match cases with [_] -> 5 | _ -> 0 in
  let splitting_mode = Refine_or {inside_nonsplit_or = false} in
  Parmatch.check_partial
    (partial_pred ?eqn_lev ~splitting_mode ~explode env expected_ty) loc cases

let check_unused ?eqn_lev env expected_ty cases =
  Parmatch.check_unused
    (fun refute pat ->
      match
        partial_pred ?eqn_lev ~splitting_mode:Backtrack_or ~explode:5
          env expected_ty pat
      with
...
let rec approx_type env sty =
  match sty.ptyp_desc with
    Ptyp_arrow (p, _, sty) ->
      let ty1 = if is_optional p
      then type_option env (newvar env) else newvar env in
      newty env (Tarrow (p, ty1, approx_type env sty, commu_ok))
  | Ptyp_tuple args ->
      newty env (Ttuple (List.map (approx_type env) args))
  | Ptyp_constr (lid, ctl) ->
      let path, decl = Env.lookup_type ~use:false ~loc:lid.loc lid.txt env in
      if List.length ctl <> decl.type_arity then newvar env
      else begin
        let tyl = List.map (approx_type env) ctl in
        newconstr env path tyl
      end
  | Ptyp_poly (_, sty) ->
      approx_type env sty
  | _ -> newvar env
...
let rec type_approx env sexp =
  ...
  | Pexp_fun (p, _, _, e) ->
      let ty = if is_optional p
      then type_option env (newvar env) else newvar env in
      newty env (Tarrow(p, ty, type_approx env e, commu_ok))
  | Pexp_function ({pc_rhs=e}::_) ->
      newty env (Tarrow(Nolabel, newvar env, type_approx env e, commu_ok))
  | Pexp_match (_, {pc_rhs=e}::_) -> type_approx env e
  | Pexp_try (e, _) -> type_approx env e
  | Pexp_tuple l -> newty env (Ttuple(List.map (type_approx env) l))
...
let check_univars env kind exp ty_expected vars =
  let pty = instance env ty_expected in
  let exp_ty, vars =
    with_local_level_iter env ~post:generalize begin fun env ->
      match get_desc pty with
        Tpoly (body, tl) ->
          ...
          let _, ty' = instance_poly env true tl body in
          let vars, exp_ty =
            instance_parameterized_type env vars exp.exp_type in
  ...
  if not complete then
    let ty_expected = instance env ty_expected in
...
let generalize_and_check_univars env kind exp ty_expected vars =
  generalize env exp.exp_type;
  generalize env ty_expected;
  List.iter (generalize env) vars;
...
let check_absent_variant env =
  iter_general_pattern { f = fun (type k) (pat : k general_pattern) ->
    ...
    | Tpat_variant (s, arg, row) ->
      ...
      let row' =
        create_row ~fields
          ~more:(newvar env) ~closed:false ~fixed:None ~name:None in
      (* Should fail *)
      unify_pat env {pat with pat_type = newty env (Tvariant row')}
...
let generalize_structure_exp env exp = generalize_structure env exp.exp_type
...
let rec type_exp ?recarg env sexp =
  ...
  type_expect ?recarg env sexp (mk_expected (newvar env))
...
and type_expect_
  ...
  let rue exp =
    with_explanation (fun () ->
      unify_exp ~sdesc_for_hint:desc env (re exp) (instance env ty_expected));
    exp
  in
  match desc with
  | Pexp_ident lid ->
  ...
  | Pexp_apply(sfunct, sargs) ->
      ...
      let type_sfunct sfunct =
        ...
        with_local_level_iter env
          begin fun env ->
            let funct =
              with_local_level_if_principal env (fun env -> type_exp env sfunct)
          ...
          ~post:
          (fun env -> wrap_trace_gadt_instances env (lower_args TypeSet.empty))
      in
      let funct, sargs =
  ...
  | Pexp_match(sarg, caselist) ->
      let arg =
        with_local_level env (fun env -> type_exp env sarg)
  ...
  | Pexp_tuple sexpl ->
      ...
      re {
        ...
        exp_loc = loc; exp_extra = [];
        (* Keep sharing *)
        exp_type = newty env (Ttuple (List.map (fun e -> e.exp_type) expl));
        exp_attributes = sexp.pexp_attributes;
        exp_env = env }
  ...
  | Pexp_variant(l, sarg) ->
      ...
      with Exit ->
        ...
        rue {
          ...
          exp_loc = loc; exp_extra = [];
          exp_type = newty env (Tvariant row);
          exp_attributes = sexp.pexp_attributes;
          exp_env = env }
      end
  | Pexp_record(lid_sexp_list, opt_sexp) ->
      ...
      let opt_exp =
        ...
        | Some sexp ->
            let exp =
              with_local_level_if_principal env
                (fun env -> type_exp ~recarg env sexp)
                ~post: generalize_structure_exp
            in
            Some exp
      in
      let ty_record, expected_type =
        ...
        | (None | Some (_, _, false)), Some (_, p', _) ->
            ...
            let ty =
              with_local_level env ~post:generalize_structure
                (fun env ->
                  newconstr env p' (instance_list env decl.type_params))
            in
            ty, opt_exp_opath
      in
      ...
      with_explanation (fun () ->
        unify_exp_types loc env
          (instance env ty_record) (instance env ty_expected));
      ...
      let opt_exp, label_definitions =
        ...
        | Some exp ->
            ...
            let unify_kept lbl =
              let _, ty_arg1, ty_res1 = instance_label env false lbl in
              ...
              | exception Not_found -> begin
                  let _, ty_arg2, ty_res2 = instance_label env false lbl in
  ...
  | Pexp_setfield(srecord, lid, snewval) ->
      ...
      let ty_record =
        if expected_type = None then newvar env else record.exp_type in
      ...
      rue {
        ...
        exp_loc = loc; exp_extra = [];
        exp_type = instance env Predef.type_unit;
        exp_attributes = sexp.pexp_attributes;
        exp_env = env }
  | Pexp_array(sargl) ->
      let ty = newgenvar() in
  ...
  | Pexp_for(param, slow, shigh, dir, sbody) ->
      ...
      let id, new_env =
        ...
        | Ppat_var {txt} ->
            Env.enter_value txt
              {val_type = instance env Predef.type_int;
               val_attributes = [];
               val_kind = Val_reg;
               val_loc = loc;
               val_uid = Uid.mk ~current_unit:(Env.get_unit_name ());
  ...
  | Pexp_constraint (sarg, sty) ->
      ...
      let cty =
        ...
        ~post:(fun env cty -> generalize_structure env cty.ctyp_type)
      in
      let ty = cty.ctyp_type in
      let (arg, ty') =
        (type_argument env sarg ty (instance env ty), instance env ty) in
      rue {
      ...
      let (arg, ty',cty,cty') =
        ...
        | None ->
            ...
            let arg, gen =
              ...
              ~post:(fun env (arg,_) -> enforce_current_level env arg.exp_type)
            in
        ...
        | Some sty ->
            let cty, ty, force, cty', ty', force' =
              with_local_level_iter env ~post:generalize_structure
                begin fun env ->
                  let (cty, ty, force) =
                    Typetexp.transl_simple_type_delayed env sty
                  and (cty', ty', force') =
            ...
            in
            begin try
              let force'' = subtype env (instance env ty) (instance env ty') in
              force (); force' (); force'' ()
            with Subtype err ->
              raise (Error(loc, env, Not_subtype err))
            end;
  ...
  | Pexp_send (e, {txt=met}) ->
      let (obj,meth,typ) =
        with_local_level_if_principal env
          (fun env -> type_send env loc explanation e met)
          ~post:(fun env (_,_,typ) -> generalize_structure env typ)
      in
      let typ =
        match get_desc typ with
        | Tpoly (ty, []) ->
        ...
        | Tpoly (ty, tl) ->
            ...
            snd (instance_poly env false tl ty)
        | Tvar _ ->
            let ty' = newvar env in
            unify env (instance env typ) (newty env (Tpoly(ty',[])));
        ...
        (path_self, _) ->
          let type_override (lab, snewval) =
            begin try
              ...
              (id, lab, type_expect env snewval (mk_expected (instance env ty)))
            with
              Not_found ->
  ...
  | Pexp_letmodule(name, smodl, sbody) ->
      ...
      let (id, pres, modl, _, body) =
        ...
        ~post: begin fun env (_id, _pres, _modl, new_env, body) ->
          ...
          (* unify_var enforces the level of env,
             expanding definitions in new_env if needed *)
          unify_var new_env (newvar env) body.exp_type
        end
      in
      re {
  ...
  | Pexp_poly(sbody, sty) ->
      let ty, cty =
        with_local_level_if_principal env
          ~post:(fun env (ty,_) -> generalize_structure env ty)
          begin fun env ->
      ...
      let exp =
        ...
        | Tpoly (ty', tl) ->
            ...
            let (exp,_) =
              with_local_level env begin fun env ->
                let vars, ty'' =
                  with_local_level_if_principal env
                    (fun env -> instance_poly env true tl ty')
                    ~post:(fun env (_,ty'') -> generalize_structure env ty'')
                in
                let exp = type_expect env sbody (mk_expected ty'') in
                (exp, vars)
              end
              ~post: begin fun env (exp,vars) ->
        ...
        | Tvar _ ->
            ...
            let exp = {exp with exp_type =
                       newty env (Tpoly (exp.exp_type, []))} in
  ...
  | Pexp_newtype({txt=name}, sbody) ->
      ...
      let body, ety = with_local_level env begin fun env ->
  ...
  | Pexp_pack m ->
      let (p, fl) =
        match get_desc (Ctype.expand_head env (instance env ty_expected)) with
          Tpackage (p, fl) ->
      ...
      rue {
        ...
        exp_loc = loc; exp_extra = [];
        exp_type = newty env (Tpackage (p, fl'));
        exp_attributes = sexp.pexp_attributes;
        exp_env = env }
  | Pexp_open (od, e) ->
  ...
  | Pexp_letop{ let_ = slet; ands = sands; body = sbody } ->
      let rec loop spat_acc ty_acc sands =
        ...
        | { pbop_pat = spat; _} :: rest ->
            ...
            let ty_acc = newty env (Ttuple [ty_acc; ty]) in
            loop spat_acc ty_acc rest
      in
      let op_path, op_desc, op_type, spat_params, ty_params,
          ty_func_result, ty_result, ty_andops =
        with_local_level_iter_if_principal env
          ~post:generalize_structure begin fun env ->
          ...
          let spat_params, ty_params = loop slet.pbop_pat (newvar env) sands in
          let ty_func_result = newvar env in
          let ty_func =
            newty env (Tarrow(Nolabel, ty_params, ty_func_result, commu_ok)) in
          let ty_result = newvar env in
          let ty_andops = newvar env in
          let ty_op =
            newty env (Tarrow(Nolabel, ty_andops,
            newty env (Tarrow(Nolabel, ty_func, ty_result, commu_ok)),
                              commu_ok))
          in
          begin try
  ...
  | Pexp_extension ({ txt = ("ocaml.extension_constructor"
      ...
      | PStr [ { pstr_desc =
          ...
          rue {
            ...
            exp_loc = loc; exp_extra = [];
            exp_type = instance env Predef.type_extension_constructor;
            exp_attributes = sexp.pexp_attributes;
            exp_env = env }
      | _ ->
...
and type_function ?(in_function : (Location.t * type_expr) option)
  ...
  let separate = !Clflags.principal || Env.has_local_constraints env in
  let ty_arg, ty_res =
    with_local_level_iter_if separate env ~post:generalize_structure
      begin fun env ->
        let (ty_arg, ty_res) =
          try filter_arrow env (instance env ty_expected) arg_label
          with Filter_arrow_failed err ->
            ...
            | Label_mismatch { got; expected; expected_type} ->
                Abstract_wrong_label
                  { got; expected; expected_type; explanation }
        ...
        let ty_arg =
          if is_optional arg_label then
            let tv = newvar env in
            begin
              try unify env ty_arg (type_option env tv)
              with Unify _ -> assert false
            end;
            type_option env tv
          else ty_arg
  ...
  re {
    ...
    exp_loc = loc; exp_extra = [];
    exp_type =
      instance env (newgenty (Tarrow(arg_label, ty_arg, ty_res, commu_ok)));
...
and type_label_access env srecord usage lid =
  let record =
    with_local_level_if_principal env ~post:generalize_structure_exp
      (fun env -> type_exp ~recarg:Allowed env srecord)
  in
...
and type_label_exp create env loc ty_expected
  ...
  *)
  let (vars, ty_arg, snap, arg) =
    ...
    with_local_level env begin fun env ->
      let (vars, ty_arg) =
        with_local_level_iter_if separate env begin fun env ->
          let (vars, ty_arg, ty_res) =
            with_local_level_iter_if separate env ~post:generalize_structure
              begin fun env ->
                let ((_, ty_arg, ty_res) as r) =
                  instance_label env true label in
                (r, [ty_arg; ty_res])
              end
          in
          begin try
  ...
  let arg =
    try
      if (vars = []) then arg
        ...
        {arg with exp_type = instance env arg.exp_type}
      end
    with first_try_exn when maybe_expansive arg -> try
      (* backtrack and try the second approach *)
      Option.iter Btype.backtrack snap;
      let arg = with_local_level env (fun env -> type_exp env sarg)
          ~post:(fun env arg -> lower_contravariant env arg.exp_type)
      in
      let arg =
        with_local_level env begin fun env ->
          let arg = {arg with exp_type = instance env arg.exp_type} in
          unify_exp env arg (instance env ty_arg);
          arg
        end
        ~post: begin fun env arg ->
          generalize_and_check_univars env "field value" arg label.lbl_arg vars
        end
      in
      {arg with exp_type = instance env arg.exp_type}
...
and type_argument ?explanation ?recarg env sarg ty_expected' ty_expected =
  ...
  match may_coerce with
    Some (safe_expect, lv) ->
      ...
      let texp =
        with_local_level_if_principal env ~post:generalize_structure_exp
          (fun env -> type_exp env sarg)
      in
      let rec make_args args ty_fun =
        match get_desc (expand_head env ty_fun) with
        | Tarrow (l,ty_arg,ty_fun,_) when is_optional l ->
            let ty = option_none env (instance env ty_arg) sarg.pexp_loc in
      ...
      and texp = {texp with exp_type = instance env texp.exp_type} in
  ...
  let type_unknown_arg (ty_fun, typed_args) (lbl, sarg) =
    ...
    let arg () =
      ...
      if is_optional lbl then
        unify_exp env arg (type_option env (newvar env));
      arg
    in
    ...
    | Tarrow (l, ty, ty_fun, com), Tarrow (_, ty0, ty_fun0, _)
      when is_commu_ok com ->
        ...
        let eliminate_optional_arg () =
          ...
          (fun () -> option_none env (instance env ty) Location.none)
        in
...
and type_construct env loc lid sarg ty_expected_explained attrs =
  ...
  let ty_args, ty_res, texp =
    with_local_level_iter_if separate env ~post:generalize_structure
      begin fun env ->
        let ty_args, ty_res, texp =
          with_local_level_if separate env begin fun env ->
            let (ty_args, ty_res, _) =
              instance_constructor env Keep_existentials_flexible constr
            in
            let texp =
              re {
            ...
            ~post: begin fun env (_, ty_res, texp) ->
              generalize_structure env ty_res;
              with_explanation explanation (fun () ->
                unify_exp env {texp with exp_type = instance env ty_res}
                  (instance env ty_expected));
            end
        in
...
and type_statement ?explanation env sexp =
  ...
  let exp = with_local_level env (fun env -> type_exp env sexp) in
  let ty = expand_head env exp.exp_type in
  if is_Tvar ty && get_level ty > Env.current_level env then
    Location.prerr_warning
      (final_subexpression exp).exp_loc
      Warnings.Nonreturning_statement;
  if !Clflags.strict_sequence then
    let expected_ty = instance env Predef.type_unit in
    with_explanation explanation (fun () ->
...
and type_unpacks ?(in_function : (Location.t * type_expr) option)
  ...
  let ty = newvar env (* remember original level *)
...
and type_cases
  ...
  with_local_level_iter_if may_contain_gadts env begin fun env ->
  let eqn_lev = Env.current_level env in
  ...
  let half_typed_cases, ty_res, do_copy_types, ty_arg' =
   (* propagation of the argument *)
    with_local_level env begin fun env ->
      ...
      let half_typed_cases =
        ...
        (fun ({pc_lhs; pc_guard = _; pc_rhs = _} as case) ->
          let htc =
            with_local_level_if_principal env begin fun env ->
              let ty_arg =
                ...
                with_local_level env ~post:generalize_structure
                  (fun env ->
                    instance env ?partial:take_partial_instance ty_arg)
              in
              let (pat, ext_env, force, pvs, unpacks) =
                type_pattern category ~eqn_lev env pc_lhs ty_arg
              in
              pattern_force := force @ !pattern_force;
              { typed_pat = pat;
            ...
            ~post: begin fun env htc ->
              iter_pattern_variables_type
                (generalize_structure env) htc.pat_vars;
            end
          in
          ...
          {htc with
           typed_pat = { pat with pat_type = instance env pat.pat_type }}
        )
        caselist in
      let patl =
    ...
    ~post: begin fun env (half_typed_cases, _, _, ty_arg') ->
      generalize env ty_arg';
      List.iter (fun { pat_vars; _ } ->
        iter_pattern_variables_type (generalize env) pat_vars
      ) half_typed_cases
    end
  in
  (* type bodies *)
  ...
  let ignore2 _ _ = () in
  let cases = with_local_level_if_principal env ~post:ignore2 begin fun env ->
    List.map
      (fun { typed_pat = pat; branch_env = ext_env; pat_vars = pvs; unpacks;
             untyped_case = {pc_lhs = _; pc_guard; pc_rhs};
             contains_gadt; _ }  ->
        (* XXXX: a careful review is needed here *)
        let ext_env = Env.copy_levels ~from:env ext_env in
        let ext_env =
  ...
  let partial =
    if partial_flag then
      check_partial ~eqn_lev env ty_arg_check loc val_cases
    else
      Partial
  in
  let unused_check delayed =
    ...
    with_level_if delayed ~level:eqn_lev env begin fun env ->
      check_unused ~eqn_lev env ty_arg_check val_cases ;
      check_unused ~eqn_lev env Predef.type_exn exn_cases ;
    end;
  ...
  ~post:(fun env ty_res' -> unify_exp_types loc env ty_res' (newvar env))
...
and type_let ?check ?check_strict
  ...
  let (pat_list, exp_list, new_env, unpacks, _pvs) =
    with_local_level env begin fun env ->
      ...
      let (pat_list, new_env, force, pvs, unpacks) =
            ...
            List.iter2
              (fun pat binding ->
                let pat =
                  ...
                  | Tpoly (ty, tl) ->
                      {pat with pat_type =
                       snd (instance_poly env ~keep_names:true false tl ty)}
                  | _ -> pat
        ...
        ~post: begin fun env (pat_list, _, _, pvs, _) ->
          ...
          iter_pattern_variables_type (generalize_structure env) pvs;
          List.iter (fun pat -> generalize_structure env pat.pat_type) pat_list
        end
      in
      let pat_list =
        List.map
          (fun pat -> {pat with pat_type = instance env pat.pat_type})
          pat_list
      in
          ...
          (fun exp_env {pvb_expr=sexp; pvb_attributes; _} pat ->
            ...
            | Tpoly (ty, tl) ->
                let vars, ty' =
                  with_local_level_if_principal env
                    ~post:(fun env (_,ty') -> generalize_structure env ty')
                    (fun env -> instance_poly env ~keep_names:true true tl ty)
                in
                let exp =
    ...
    ~post: begin fun env (pat_list, exp_list, _, _, pvs) ->
      List.iter2
        (fun pat (exp, _) ->
          if maybe_expansive exp then lower_contravariant env pat.pat_type)
        pat_list exp_list;
      iter_pattern_variables_type (generalize env) pvs;
      List.iter2
        (fun pat (exp, vars) ->
          match vars with
          | None ->
  ...
  let rec loop env let_sarg rev_sands expected_ty =
    ...
    | { pbop_op = sop; pbop_exp = sexp; pbop_loc = loc; _ } :: rest ->
        let op_path, op_desc, op_type, ty_arg, ty_rest, ty_result =
          with_local_level_iter_if_principal env begin fun env ->
            let op_path, op_desc = type_binding_op_ident env sop in
            let op_type = instance env op_desc.val_type in
            let ty_arg = newvar env in
            let ty_rest = newvar env in
            let ty_result = newvar env in
            let ty_rest_fun =
              newty env (Tarrow(Nolabel, ty_arg, ty_result, commu_ok)) in
            let ty_op =
              newty env (Tarrow(Nolabel, ty_rest, ty_rest_fun, commu_ok)) in
            begin try
              unify env op_type ty_op
            with Unify err ->
